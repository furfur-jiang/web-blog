<template>
  <el-container class="container">
    <div class="btn_groups">
      ...
      >
        <el-button circle @click="collectBtn" :disabled="collectBtnDisabled">
    ...
    <el-card class="article">
      ...
      <div class="user-card">
        <div class="user-message">
          <img v-if="!blogUser.headPortrait" src="@/assets/head1.png" />
          <img v-else :src="cHeadPortrait" />
          <div>
            <span class="user-name">{{ blogUser.name }}</span>
            <span class="user-time">
              {{ createTime }} 阅读{{ article.visitCount }}
            </span>
...
getArticle() {
    ...
    getUserMoreById() {
      this.$http
        .get(`/web_blog/userExpansion/getUserByUserId?userId=${this.blogId}`)
        .then((res) => {
          if (res.data.code === 0) {
